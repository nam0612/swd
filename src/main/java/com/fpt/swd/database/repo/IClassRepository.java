...
public interface IClassRepository extends JpaRepository<Class, Integer> {
    @Query("SELECT new com.fpt.swd.database.dto.Class.GetClassVer2Dto(c.id,  c.name,  c.description,  c.semester_id,  c.teacher_id,  c.subject_id,  c.start_date,  c.end_date,  c.status,  c.created_by,  c.updated_by,  s.code,  u.firstName,  u.lastName,  sj.subject_code,  sj.subject_name) " +
            "from Class as c " +
            "JOIN FETCH Setting as s ON c.semester_id = s.id " +
            "JOIN FETCH User as u ON c.teacher_id = u.id " +
            "JOIN FETCH Subject as sj ON c.semester_id = sj.id")
